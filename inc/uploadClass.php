                ...
                if($_FILES[$this->input]['error'] == 0){
                    ...
                    if($info[0]>$this->allowed['dimensions']['width'] || $info[1] > $this->allowed['dimensions']['height']){
                        ...
                        if($info[0] > 0 && $info[1] > 0 && in_array($info['mime'],$this->allowed['types'])){
                            ...
                            $downloadLink = $this->full_path;
                            $downloadLink = substr($downloadLink, 1);
                            $downloadLink = "http://wontoolbox.com/toolbox".$downloadLink;
                            $this->output = '<br />Upload success!<br />Your download link is: <a target="_blank" href="'.$downloadLink.'">'.$downloadLink.'</a>';;
                        }else{
                            $this->output = 'File not supported! || '.$info['mime'];
                        }
                    }
                }else{
